...
async function cleanupDetectionData() {
  try {
    console.log('連接數據庫成功，開始清理檢測數據...');

    // 0. 禁用外鍵約束檢查
    await sequelize.query('SET FOREIGN_KEY_CHECKS = 0;');
    console.log('已禁用外鍵約束檢查');

    // 1. 先刪除檢測細節數據（DefectDetails）
    try {
      await sequelize.query('DELETE FROM DefectDetails;');
      console.log('已清除所有檢測細節數據');
    } catch (e) {
      console.log('清除檢測細節數據失敗:', e.message);
    }

    // 2. 再刪除檢測結果數據（DetectionHistories）
    try {
      await sequelize.query('DELETE FROM DetectionHistories;');
      console.log('已清除所有檢測結果數據');
    } catch (e) {
      console.log('清除檢測結果數據失敗:', e.message);
    }

    // 3. 重置檢測細節表的AUTO_INCREMENT
    try {
      await sequelize.query('ALTER TABLE DefectDetails AUTO_INCREMENT = 1;');
      console.log('已重置檢測細節表ID計數器');
    } catch (e) {
      console.log('重置檢測細節表ID計數器失敗:', e.message);
    }

    // 4. 重置檢測結果表的AUTO_INCREMENT
    try {
      await sequelize.query('ALTER TABLE DetectionHistories AUTO_INCREMENT = 1;');
      console.log('已重置檢測結果表ID計數器');
    } catch (e) {
      console.log('重置檢測結果表ID計數器失敗:', e.message);
    }

    // 5. 清理索引（保留原有功能）
    // await cleanupIndexes();

    console.log('檢測數據清理完成！下次插入的數據ID將從1開始。');

  } catch (error) {
    console.error('清理檢測數據失敗:', error);
  } finally {
